...
const Achievement = () => {
  ...
  const [loading, setLoading] = useState(true); // Mengubah loading ke true di awal
  ...
  useEffect(() => {
    const fetchAchievements = async () => {
      try {
        ...
        setAchievement(achievementsData);
        setLoading(false); // Menetapkan loading ke false setelah pengambilan data selesai
      } catch (error) {
        console.error("Error fetching achievements:", error.message); // Menangani kesalahan dengan pesan yang lebih deskriptif
        setLoading(false); // Pastikan loading diatur ke false dalam kasus kesalahan
      }
    };
